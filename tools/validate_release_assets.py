"""Static release-asset validation for the Swin2SR super-resolution DIMER pipeline.
...
EXPECTED_OUTPUTS = (
    ...
    'outputs/swin2sr_super_resolution_output.png',
)
...
CODE_MARKERS = (
    ...
    'from swin2sr_super_resolution_pipeline import MODEL_ID, MODEL_KEY, MODEL_REVISION, '
    'Swin2SRPipeline, psnr, stage_missing_files, verify_snapshot',
    ...
    "print({'ceilings': {'UPSCALE': UPSCALE, 'MIN_INPUT_SIDE': MIN_INPUT_SIDE, "
    "'MAX_INPUT_SIDE': MAX_INPUT_SIDE}})",
    'lr_image = hr_reference.resize((HR_WIDTH // UPSCALE, HR_HEIGHT // UPSCALE), Image.Resampling.BICUBIC)',
    "'model_vs_self_made_reference': psnr(sr_array, hr_array)",
    "'bicubic_baseline_vs_self_made_reference': psnr(bicubic_array, hr_array)",
...
MARKDOWN_MARKERS = (
    '**Capability:** 2× single-image super-resolution (classical SR)',
    '**No adaptation occurs:**',
    'The 512 px ceiling was lowered from 1024 px by the repository owner on 2026-09-12',
    '160 s for a 1024×1024 input on CPU against 34 s for 512×512',
    'It is computed only when a high-resolution reference exists',
    'self-made reference, not a benchmark',
    'As a **meaningful baseline** the same PSNR is computed for a plain bicubic 2× resize',
    'scales other than 2×, compressed-image or real-world (blind) restoration',
)
